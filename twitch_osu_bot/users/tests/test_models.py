...
class TestOsuUsername(TestCase):
    ...
    def test_send_confirmation(self, mock_info, mock_create):
        ...
        self.assertEqual(
            self.osu_user.send_confirmation(request=request, message=True),
            confirmation
        )
...
class TestOsuUsernameConfirmation(TestCase):
    ...
    def test_key_expired(self, mock_now):
        ...
        mock_now.return_value = self.confirmation.created + timedelta(minutes=14, seconds=59)
        self.assertFalse(self.confirmation.key_expired())
    ...
    def test_confirm(self, mock_expired):
        ...
        self.assertRaises(
            OsuUsernameConfirmation.ConfirmationExpired,
        ...
        self.assertRaises(
            OsuUsernameConfirmation.AlreadyVerified,
            self.confirmation.confirm
        )
